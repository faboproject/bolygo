...
public class MyJoglCanvasStep6 extends GLCanvas implements GLEventListener {
	...
	public void init(GLAutoDrawable drawable) {
		...
		// Load earth texture.
		try {
			InputStream stream = getClass().getResourceAsStream("ea.jpg");
			TextureData data = TextureIO.newTextureData(
					drawable.getGLProfile(), stream, false, "jpg");
		...
		try {
			InputStream stream = getClass().getResourceAsStream("solar.png");
			TextureData data = TextureIO.newTextureData(
	...
	public void display(GLAutoDrawable drawable) {
		...
		satelliteAngle = (satelliteAngle + 0.5f) % 360f;
		final float distance = 10.000f;
		final float x = (float) Math.sin(Math.toRadians(satelliteAngle))
				* distance;
		final float y = (float) Math.cos(Math.toRadians(satelliteAngle))
				* distance;
		final float z = 0;
